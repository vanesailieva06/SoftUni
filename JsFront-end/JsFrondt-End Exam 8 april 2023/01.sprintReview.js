function solve(input) {
  ...
  function changeStatus(tokens) {
    ...
    if (!sprintMap.hasOwnProperty(assignee)) {
      console.log(`Assignee ${assignee} does not exist on the board!`);
    } else if (sprintMap[assignee].taskId !== taskId) {
      console.log(`Task with ID ${taskId} does not exist for ${assignee}!`);
    } else {
      for (let i = 0; i < sprintMap[assignee].length; i++) {
        if (sprintMap[assignee][i].taskId === taskId) {
          let oldStatus = sprintMap[assignee].status;
          statusPoints[oldStatus] -= sprintMap[assignee].estimatedPoints;
          sprintMap[assignee].status = newStatus;
          statusPoints[newStatus] += sprintMap[assignee].estimatedPoints;
        }
      }
    }
  }
